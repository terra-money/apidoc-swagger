...
function createFieldArrayDefinitions(fieldArray, definitions, topLevelRef, defaultObjectName) {
    ...
    for (var i = 0; i < fieldArray.length; i++) {
        ...
        var nestedName = createNestedNameWithDelemeter2(parameter.field);
...
function createNestedNameWithDelemeter2(field) {
    var propertyName = field;
    var objectName;
    var propertyNames = field.split("2");
    if (propertyNames && propertyNames.length > 1) {
        propertyName = propertyNames[propertyNames.length - 1];
        propertyNames.pop();
        objectName = propertyNames.join("2");
    }
...
function checkForRootObject(fields, defName) {
    ...
    if (rootPropertyCount > 1) {
        ...
        for (var field of fields) {
            ...
            field.field = field.field.split('.').join('2');
        }
    }
...
function createSchema(fields, definitions, defName, objName) {
    // console.log('LOLOLO ', defName, objName)
    // console.log(fields)
